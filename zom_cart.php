    ...
    <h1 class="mt-5">Cart</h1> 
        <div class="row">
            <div class="col-8">
                ...
                {
                    $amount = $amount + $row['price'] * $row['quantity']+5;
                        echo'<div class="card" id="product_card'.$row['product_id'].'">
                        <div class="row">
                            <div class="col-2">
                                <img src="'.$row['image'].'"width="100%" height="100px">
                            </div>
                            <div class="col-6">
                                <h5>'.$row['name'].'</h5>
                            ...
                            <div class="col-4">
                                <button class="btn btn-danger btn-sm plus-one"data-id="'.$row['product_id'].'"><b>-</b></button>
                ...
                <a href="zom_place_order.php" class="btn btn-lg btn-danger" style="float: right;">Checkout</a>';
                }
                ?>
            </div>
        </div> 
...
<script type="text/javascript">
    ...
    $('.plus-one').click(function() {
                ...
                $.ajax({
                    url: 'zom_update_product_quantity.php?product_id=' + product_id + '&quantity=' + quantity + '&sign='+ sign,
                    method: 'GET',
                    success: function(data) {
                        if (sign === '+') {
                            ...
                            if ((Number(quantity) - 1) === 0){
                                ...
                                $.ajax({
                                        url: 'zom_remove_from_database.php?product_id=' + product_id,
                                        method: 'GET',
                                        success:function(data){
                                             // remove the product from the ui
                                            // console.log(data);
                                            $('#product_card' + product_id).hide();
                                        },
